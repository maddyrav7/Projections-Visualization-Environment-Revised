...
public class SimplifiedMesh
{
    ...
    public void SimplifyLines(MeshDataTool mdt)
    {
        ...
        // Loop through each MDT edge
        for (int edge = 0; edge < mdt.GetEdgeCount(); edge++)
        {
            // Find the local (unique) indices of points that form an edge (so duplicates can be compared)
            ...
            // Also only add line if both faces that the line's local equivalent is a part of are not parallel
            ...
            else
            {
                ...
                // Find which index to add to the local mapping lists
                if (pairAIndex != -1)
                {
    ...
    {
        // For loop for running through all unique pairs of faces (order does not matter)
